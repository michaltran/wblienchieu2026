...
export default function DoctorCard({ doctor, className }: DoctorCardProps) {
  ...
  const initials = doctor.name
    ? doctor.name.split(" ").map((n) => n[0]).join("").slice(-2)
    : "BS";
  ...
  return (
    <div className={cn("bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full", className)}>
      <div className="p-6 flex flex-col items-center text-center flex-grow">
        ...
        <p className="text-sm text-slate-500 mb-3 font-medium min-h-[1.25rem]">{doctor.title}</p>
        ...
        {/* Meta / Tags */}
        <div className="flex flex-wrap gap-1 justify-center mb-6 min-h-[1.5rem]">
        ...
        <div className="mt-auto pt-4 border-t border-slate-50 w-full grid grid-cols-2 gap-3">
