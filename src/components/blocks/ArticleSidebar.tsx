...
export default function ArticleSidebar() {
  ...
  const categories = ["Tin tức", "Sức khỏe", "Hướng dẫn", "Hoạt động đoàn thể"];
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <ul className="space-y-2">
          {categories.map((cat, idx) => (
            <li key={idx}>
              <Link to={`/bai-viet?category=${cat}`} className="block px-3 py-2 rounded-lg hover:bg-slate-50 text-slate-600 hover:text-primary transition-colors text-sm">
                {cat}
              </Link>
            </li>
          ))}
